...
<section class="content">
    <div class="container-fluid">
        ...
        <td>
            <a href="{{ route('kantor.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>
            <form action="{{ route('kantor.destroy', $item->id) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Delete this kantor?')">Delete</button>
            </form>
        </td>
                            <td>
                                <a href="{{ route('kantor.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>
