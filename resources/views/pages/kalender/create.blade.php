...
@section('content')
    <x-page :title="__('Add Event')" :breadcrumb="__('Work Calendar')">
        <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="tipe" id="tipe" class="form-control @error('tipe') is-invalid @enderror" required>
                            <option value="hari_libur" {{ old('tipe') == 'hari_libur' ? 'selected' : '' }}>{{ __('Holiday') }}</option>
                            <option value="meeting" {{ old('tipe') == 'meeting' ? 'selected' : '' }}>{{ __('Meeting') }}</option>
                            <option value="acara" {{ old('tipe') == 'acara' ? 'selected' : '' }}>{{ __('Event') }}</option>
                            <option value="lainnya" {{ old('tipe') == 'lainnya' ? 'selected' : '' }}>{{ __('Other') }}</option>
                        </select>
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <select name="repeat_type" id="repeat_type" class="form-control @error('repeat_type') is-invalid @enderror">
                                <option value="never" {{ old('repeat_type', 'never') == 'never' ? 'selected' : '' }}>{{ __('Never') }}</option>
                                <option value="weekly" {{ old('repeat_type') == 'weekly' ? 'selected' : '' }}>{{ __('Weekly') }}</option>
                                <option value="monthly" {{ old('repeat_type') == 'monthly' ? 'selected' : '' }}>{{ __('Monthly') }}</option>
                                <option value="yearly" {{ old('repeat_type') == 'yearly' ? 'selected' : '' }}>{{ __('Yearly') }}</option>
                            </select>
