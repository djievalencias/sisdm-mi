...
class UserController extends Controller
{
    public function index(Request $request)
    {
        ...
        $users = User::where('is_archived', false)->paginate($perPage);
    ...
    public function show($id)
    {
        ...
        if (!$user) {
            return response()->json([
                'message' => 'Pengguna tidak ditemukan atau telah diarsipkan.'
            ], 404);
        }
    ...
    {
        $user = User::where('is_archived', false)->with('shifts')->find($userId);

        if (!$user) {
            return response()->json(['message' => 'Pengguna tidak ditemukan atau telah diarsipkan'], 404);
        }
